...
- **⚠ AI へテキストを渡す経路は2つあり、性質が正反対**(2026-07-27 に実測):
  ...
  会話の指定を捨てた(2026-07-27 / シロさんの判断)。`ChatGptLink` と
  設定の会話URLは**ChatGPT アプリが無い端末向けの退避経路**として残っている。
...
- **次にやること**(詳細は WORKLOG 冒頭の再開ポイント):
  1. **2026-07-27 の変更を実機で通す**(一部のみ確認済み)。
     ✅ `> ヨスガへ現状報告` で端末ログに `CONTEXT … (+ PROMPT 1222)` が出た
     (**指示文の同梱は実機で通った**)。
     未確認: 設定に会話URL → 夜の2ボタン → `> 回答JSON → Hub` →
     Vault の Frontmatter を目で見る。
...
- **夜のボタン2つ**(2026-07-27): `> 観察日誌を要求` / `> ヨスガへ総括を要求`。
  ...
  **朝と同じ共有インテントで渡す**ので、指示文は必ず入力欄に入る。
  ...
  → `YosugaPrompt.NIGHT_HEADER` が**役割**と「**材料が無いなら一度だけ聞き返す・
  推測で埋めない**」を毎回同梱する。これが落ちると、ヨスガは自分が何者かを
  知らないまま、それらしい嘘の日誌を書いて Obsidian に残す(**動くので気づけない**)。
