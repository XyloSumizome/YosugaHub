...
## ▶▶ 次回の再開ポイント(2026-07-25 時点・ここだけ読めば再開できる)
...
2. **ノートの上書き反映を実機で確認**。どれかのゲームで `.yosuga/notes/` の既存ノートを
   修正して push → `GitHub → Obsidian` → **「更新 1件」が出れば本物**。
...
4. **クリップボード自動入力を実機で確認**。何かJSONをコピーしてから `回答JSON → Hub` を押す。
   空のままなら配線の問題(ロジックは単体テスト済み)。

### 未検証・残っている宿題

- **ノートの上書き反映が実機未確認**(上記 2.)。単体テストでは押さえてある。
- **クリップボード自動入力が実機未確認**。エミュレータはクリップボードへ
  文字列を入れる手段が無く確認できなかった(`cmd clipboard` 未実装)。
- **カレンダーが0件**。エミュレータに Google アカウントが無く `READ_CALENDAR` も未許可。
  AVD は Play Store 対応イメージ(`google_apis_playstore_ps16k`)なので、
  **アカウントを追加すればエミュレータでも同期できる**。実機に回す判断をした。
