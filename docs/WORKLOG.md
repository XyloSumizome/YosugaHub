...
3. **レコルの「巡回」を実レコルで試す**(2026-07-25 追加)。
   `docs/recoru_prompt.md` を**カスタムGPTの Instructions へ貼り直してから**、
   「巡回」と一言。手順は同ファイルの「巡回の回し方」。
   走り切れば、次は **保存口(`inbox.php` + `PULL INBOX`)** を作る段。
...
- **レコルの「保存まで自動」は未実装**(2026-07-25)。プロンプト側(巡回)だけ先に入れた。
  残りは `inbox.php`(⚠ `upload.php` の流用は禁止・`projects.json` を潰す)+
