...
### ☀ 朝の確認リスト(2026-07-24 朝用 / 前夜の続き)

前夜の状態: トークン作り直し済み・レコル(カスタムGPT+Actions)から index 取得成功・
レコルが返した分類JSON(confidence 0.25)は**まだ取り込んでいない**(ファイル化できず中断
→ 対応として「貼り付けて取り込む」を実装済み)。

1. アプリを**ビルドし直して**エミュレーターへ(貼り付け取り込みは昨夜の追加分)
2. ホームかヨスガ画面の「**回答JSONを貼り付けて取り込む**」に、レコルの会話に残っている
   分類JSONをコピーして貼る → 「文書 1 件の分類を受け取りました」+「サーバーへ反映しました」
3. 記録タブ → 文書 → test が「確認待ち」→ 承認 → 「分類済み」になれば **v4.1 が一周完了**
4. レコルの Instructions を `docs/recoru_prompt.md` に差し替え(まだなら)→
   「**Morning Brief を作って**」→ 出てきたものをヨスガ(通常ChatGPT + `docs/yosuga_prompt.md`)へ貼る
5. 余裕があれば: `./gradlew connectedDebugAndroidTest`(マイグレーション7件の実行)
