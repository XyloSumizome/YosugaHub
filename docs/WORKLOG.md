...
1. **ChatGPT の共有シートに Yosuga Hub が出るか実機で確かめる**。
   **ここが没なら「ヨスガ/レコル → Hub」の入り口の設計が変わる**ので最優先。
2. **クリップボード自動入力を実機で確認**。何かJSONをコピーしてから `回答JSON → Hub` を押す。
   空のままなら配線の問題(ロジックは単体テスト済み)。
   **1 が没だった場合、ここが唯一の入り口になる。**
3. **AI 側の指示を貼り直す**(2026-07-25 にいくつも直したため)。
   **1 の結果で文面が変わりうるので後に回した。**
...
4. **カレンダーを実データにする**。実機で `READ_CALENDAR` を許可する。
5. **Obsidian + Remotely Save で Dropbox 同期**。Vault は `/sdcard/Obsidian/Yosuga` に
   作ってあり、Hub からの書き込みは確認済み。あとは Obsidian 側から開いて同期を設定するだけ。

### 未検証・残っている宿題

- **クリップボード自動入力が実機未確認**。エミュレータはクリップボードへ
  文字列を入れる手段が無く確認できなかった(`cmd clipboard` 未実装)。**実機なら確認できる。**
