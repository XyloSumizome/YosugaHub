...
### 2026-07-26: 実機が入り、宿題がまとめて片付いた

**実機 Pixel 10a / Android 17 を導入した。** エミュレータでは確かめられなかった項目を通した。

| 済んだこと | 結果 |
|---|---|
| マイグレーションテスト(実機) | **5件成功**。長年の「未検証」が解消 |
| 実機の運用データ投入 | プロジェクト4件 + GitHubトークン + 取得成功 |
| Vault への書き込み | **新規11件**。`Games/<表示名>/<Decisions\|Development Logs>/` へ正しく振り分け |
| **ノートの上書き反映** | **更新1件・枝番なし**。実ファイルで裏取り済み(**risk-010 解消**) |
...
`files/imports/` に**同一サイズ(1867バイト)で2本**残った。
懸念していた「ChatGPT の共有はリンクだけ送ってくる」は**起きなかった**。
`docs/recoru_prompt.md` の「共有 → Yosuga Hub。貼り付けは不要」は**書き換え不要**。

⚠ **生成したサーバートークンがコピーできない不具合を直した**(`f4c42f9`)。
トークン欄は `PasswordVisualTransformation` でマスクしているため、Android の仕様で
**欄から選択してもコピーできない**(貼り付けしか出ない)。
生成できるのに取り出せず、`config.php` へ持ち出せない行き止まりだった。
生成時に自動でクリップボードへ入れ、「トークンをコピー」ボタンも出すようにした。

⚠ **レコルの「巡回」がウェブ検索を始めて遅くなる件**。
`recoru_prompt.md` には検索を促す記述が**1つも無い**。原因は
**カスタムGPT側でウェブ検索機能がONになっていること**。「巡回」という語を
「外部サイトを回る」と解釈している。**GPT編集画面で機能をOFFにするのが本命**。
指示側にも「ウェブを検索しない」を入れた(7641字 / 8000)。

⚠ **実機=運用機 / エミュレータ=テスト機 に分けた。**
`connectedDebugAndroidTest` はアプリをアンインストールするので、
**運用データを入れる前に一度だけ**流した。以後この実機では流さない。

⚠ **`server/config.php` に本番トークンが平文で入っており、public リポジトリで公開されていた。**
`config.example.php`(ひな形)を追跡し、`config.php` は `.gitignore` へ移した。
**公開されたものは取り消せない**ので、修復はトークンの作り直しで行った(シロさんが実施)。
救いは**ロリポップのドメインがリポジトリのどこにも書かれていない**こと。
詳細は `.yosuga/notes/2026-07-26-server-token-in-public-repo.md`。

- **Vault の場所**: `/sdcard/Obsidian/Yosuga`(共有ストレージ)。Obsidian アプリはまだ未導入。
- **Hub 自身をプロジェクトとして登録した**(`yosuga-hub` / 表示名 `YosugaHub`)。
  保留していた論点だが、上書き検証で両側を制御できる対象が要ったため確定させた。

### いまアプリはどういう状態か

- **設計は v5「Obsidian ブリッジ」**(`yosuga_hub_design_v5_obsidian_bridge.md`)。
  Hub は AI ではなく「情報を運ぶハブ」。知識の正本は Obsidian の Markdown。
- **v5 Phase 1〜3 実装済み・実機で通し成功**。GitHub 実通信も成功。
- テスト **385件** 通過 / マイグレーションテスト **5件** 通過。Room は **v8**。新規ライブラリなし。
- ビルドは Windows 側 JBR
  (`cmd.exe /c "set JAVA_HOME=C:\Program Files\Android\Android Studio\jbr&& gradlew.bat ..."`)。
- **GitHub へ push 済み**(2026-07-25 に初回 push。それまで108コミットが未バックアップだった)。
  リポジトリは **public**。
- **Hub 自身も1プロジェクトとして扱う**。`.yosuga/`(project/status/status.md/schema-version)
  と `.yosuga/notes/` を持つ。更新規則は `CLAUDE.md` の「Yosuga Hub Data Provider Rules」。
...
- **ヨスガ**(通常ChatGPT・**カスタムGPTではない**) = 会話の相棒。**観測日記を書く**。
  Instructions 欄が無いので**メモリに覚えさせる**。
  ⚠ **観察日記の文体規則はヨスガのメモリ側が正**。`docs/yosuga_prompt.md` には
  **Hub へ取り込む形式だけ**を書く(二重に書かない)。
- **レコル**(カスタムGPT+Actions) = 情報の仕分け。日記は書かない。
  会話を開いて一言送れば**合図なしで巡回**する。
  ⚠ Instructions は **8000字上限**。超えると**黙って末尾が切れる**。
  編集したら `python3 scripts/measure_prompts.py` で測る(現在 7570字)。
- 各ゲームの **Claude Code** = `.yosuga/notes/` と `.yosuga/status.json` を書く。
  **訂正は書き換えてよい**(2026-07-25〜。Hub が上書き反映する)。
...
1. **AI 側の指示を貼り直す**(⚠ **朝の経路を変えたので必須**)。
   - **ヨスガ**: `docs/yosuga_prompt.md`。**`# 朝の近況(Hubから直接届く)` が新しい節**。
     Morning Brief を待つのではなく、**現況JSONを読んで自分で1日の入り口を作る**。
     カスタムGPTではないので**メモリに覚えさせる**(2840字)。
   - **レコル**: `docs/recoru_prompt.md`(6339字 / 8000)。Morning Brief の節を削除した。
     **会話のきっかけからも `Morning Brief` を外す**。ウェブ検索機能は OFF のまま。
2. **実運用を1日通す**。`> 朝の準備` → 作業 → 観測日記 → 取り込み。
3. **現況JSONの量を実地で見る**(約31KB)。重すぎるなら `statusMarkdown` を削る。
...
- **`questionsForYosuga` と `projects.decisions` に日付が無い**。文字列配列なので持てない。
  ゲーム側の互換が切れるため保留。必要になったら `recentChanges` と同じ手当てをする。
- **レコル自身がサーバーへ保存する経路は未実装**。共有が通れば不要かもしれない。
  作るなら `inbox.php`(⚠ `upload.php` の流用は禁止・`projects.json` を潰す)。
- 将来: MCP(v4 Phase4)。

### 落とし穴(今日踏んだもの)

- **`connectedDebugAndroidTest` はアプリをアンインストールする**。
  Room の一時DBは無関係だが、**実行後にアプリのデータが全部消える**
  (プロジェクト・トークン・Vault選択)。トークンは Keystore の鍵ごと消えるので**再入力必須**。
- **UI を変えると文言の中の画面名が取り残される**。今日だけで3回起きた。
  コンポーネントの置換は機械的に検出できるが、**文字列の中の画面名は誰も検出しない**。
- **`text/*` を KDoc に書くとビルドが落ちる**。Kotlin はブロックコメントが入れ子になるため。
- **Markdown の入れ子フェンス**(```` ```json ```` を ``` の中)は、
  コピー時に**静かに途切れる**。インデント記法にする。
...
- エミュレータには3プロジェクト登録済み・GitHub 取得済み(recentChanges: 5 / 6 / 10件)。
- **Vault は未選択**なので、Obsidian への書き込みは全部スキップされる。
- 承認待ちが数件(検証で入れたゴミを含む)。`REVIEW` で棄却してよい。
- 使い方の一覧は **`docs/how-to-use.html`**(ブラウザで開く)。
