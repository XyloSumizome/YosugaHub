...
- ビルドは Windows 側 JBR
  ...
  ⚠ `clean assembleDebug testDebugUnitTest installDebug` を一度に流すと、端末が
  外れているとき `installDebug` で落ちて**テスト結果が残らない**。分けて流すほうが安全。
- **⚠ GitHub へ 3コミット未 push**(`a012c4d` / `90f2154` / `97516c7`)。
  push は毎回シロさんの判断(public リポジトリ)。
- **Vault は `/sdcard/Obsidian/Yosuga`**(共有ストレージ)。Obsidian + Remotely Save で
  Dropbox(`/Apps/remotely-save/Yosuga`)へ同期している。
- WSL から実機を触れる:
  `/mnt/c/Users/note_/AppData/Local/Android/Sdk/platform-tools/adb.exe`(Windows の adb)。
  ビルド・インストール・logcat・`run-as` でのデータ確認まではできる。**画面は触れない。**

### コンソールのコマンド(2026-07-27 に改称)

```
> ヨスガへ現状報告    カレンダー取込 → GitHub取得 → Vault書込 → サーバー同期
                      → 指示文 + 現況JSON が ChatGPT へ届く → 送信
> GitHub → Obsidian
> 会話 → Obsidian
> 過去情報をヨスガへ   現況(既定)+ 選べば Vault の過去ログ
> 回答JSON → Hub
> 観察日誌を要求       ← 夜
> ヨスガへ総括を要求   ← 夜
> REVIEW
...
### 2026-07-27 に何が変わったか(1日で3波あった)

**第1波: レコル廃止 + 指示文を Hub 同梱へ + 夜の2ボタン**(`6dd85e2`)

- レコルの仕事3つのうち2つは消えており、残る1つも元はヨスガの仕事だった
- 指示文の正本を **`data/prompt/YosugaPrompt.kt`** へ。ChatGPT のメモリに頼らない
- セッション記録 `session[]` は**承認を挟まず Obsidian へ直接**書く

**第2波: 実機導入 + 改称 + 用語統一**(`a012c4d`)

- 実機へ入れた。**データは保持**(`installDebug` は上書き更新)
- ✅ **指示文の同梱を実機で確認**——端末ログに `CONTEXT … (+ PROMPT 1222)`
- コンソール4項目を改称。日記の呼称3通りを **観察日誌** へ統一(71箇所)
- `measure_prompts.py` の**静かな壊れ**を修正(順序決め打ち → 名前で取り出す)

**第3波: 渡し方の作り直し**(`97516c7`)

シロさんの指摘「チャット欄にプロンプトが貼られてないんじゃないかな」から始まった。

| | 朝 | 夜(旧) | 夜(新) |
|---|---|---|---|
| 経路 | `ACTION_SEND` | URL の `?q=` | **`ACTION_SEND`** |
| 上限 | 約1MB | 2000字(日本語 約200字) | 約1MB |
| 会話の指定 | できない | できる | **できない** |
| 入力欄 | 入る | **入らなかった** | **入る** |

**「入力欄に必ず入ること」と「会話を指定できること」は両立しない。** 前者を取った。

- → 夜は**毎回まっさらな新しい会話**が開く。`YosugaPrompt.NIGHT_HEADER` が
  役割と材料の在りかを毎回同梱する
- **設定「ヨスガの会話」は撤去**。共有インテント化で実質使われない欄になったため
- **聞き返しをやめた**。材料は「この会話が始まる前までの今日のやりとり」で、
  ChatGPT の履歴から拾わせる。⚠ **捏造の禁止だけは残す**
  (履歴参照が効かないと手元が空になり、嘘の日誌が承認なしで Obsidian に残る)

### 役割(AI は1体だけ)

- **ヨスガ**(通常ChatGPT・**カスタムGPTではない**) = 会話の相棒であり、
  **Hub と話す唯一のAI**。観察日誌・セッション記録・タスク・知識アイテムを
  すべて回答JSONで直接出す。
  ⚠ **指示文の正本は `data/prompt/YosugaPrompt.kt`**(メモリではない)。
  Hub が毎回同梱するので、貼り直し作業はもう無い。
  ⚠ メモリに残るのは**観察日誌の文体規則だけ**(`「観察日記 作成ルール」`)。
  Hub 側へ移すなら `YosugaPrompt.DIARY_STYLE` に入れる(空のあいだはメモリが正)。
  ⚠⚠ **`「観察日記 作成ルール」` という文字列は改名してはいけない。**
  メモリにある規則名そのもので、指示文が名指しで参照している。
  用語を「観察日誌」へ統一したときも、ここだけは意図的に残した。
- **レコル** = **廃止**(2026-07-27)。設備(ロリポップ同期・Actions・`openapi.yaml`・
  サーバーPHP・`documents`/`classifications`)は**残してある**が、日常経路には出てこない。
...
1. **夜の経路を実機で通す**(`risk-017`。ここだけが未検証)。
   1. `> 観察日誌を要求` → 新しい会話が開き、**指示文が入力欄に入っているか**
   2. そのまま送信 → **質問せずに書き始めるか**(2026-07-27 に聞き返しを外した)
   3. ⚠ **空っぽの日誌や「分かりません」だらけが返ってきたら、ChatGPT の
      「チャット履歴を参照」がオフの可能性が高い。** その場合ヨスガは今日の会話を
      見られないので、設定を確認する。ここが今の設計の**唯一の前提**
   4. `> ヨスガへ総括を要求` も同様
   5. 返ってきた JSON → `> 回答JSON → Hub`
      → 日誌は REVIEW へ / **セッション記録は Obsidian へ直接**
   6. Obsidian で `Conversations/Yosuga/2026-07-27-session.md` の
      **Frontmatter を目で見る**。⚠ **壊れていても Obsidian は黙って無視する**
      (検索から静かに消える)ので、自動では気づけない。目視でしか分からない
2. **観察日誌の文体規則**を Hub へ移すか決める(`YosugaPrompt.DIARY_STYLE`)。
   移すなら `docs/yosuga_prompt.md` の「文体はここに書かない」節も同時に直す
   ——**二重に書かない**のがこの規則の要点なので。
...
| | 状態 |
|---|---|
| 夜の2ボタン(入力欄・聞き返さないか) | **未検証** |
| ChatGPT の履歴参照が効いているか | **未確認**(効かないと夜の経路が成立しない) |
| 回答JSON の取り込み(日誌 / セッション記録) | **未検証** |
| Obsidian の Frontmatter | **未検証**(目視でしか分からない) |
| `DIARY_STYLE` に文体を移すか | **判断待ち** |
| push するか | **判断待ち**(3コミット) |
...
| `6dd85e2` | **レコル廃止 + 指示文を Hub 同梱へ + 夜の2ボタン**(2026-07-27) |
| `e6f1d7a` | `sourceCommit` と WORKLOG の SHA を実際の値へ |
| `a012c4d` | **コンソール4つの表示名を改める + 夜の依頼を共有インテントへ**(2026-07-27) |
| `90f2154` | `sourceCommit` と WORKLOG の SHA を実際の値へ |
| `97516c7` | **会話URL設定を撤去 + 夜の依頼で聞き返させない**(2026-07-27) |
...
## 2026-07-27(その2): 実機へ導入 + コンソールの表示名を改める

### 実機へ入れた

clean ビルドで単体テスト **429件・失敗0**、Pixel 10a へ `installDebug`。
`firstInstallTime` が 2026-07-26 のまま = **上書き更新**なので、
プロジェクト・GitHubトークン・Vault選択・サーバー同期の設定は**すべて残った**。

DataStore を覗いて確かめた点(値は見ず**キー名だけ**。トークンが入っているため):

| キー | 状態 |
|---|---|
| `github_token_encrypted` / `obsidian_vault_uri` / `sync_base_url` / `sync_token_encrypted` | 残存 → 再設定不要 |
| `yosuga_conversation_url` | **無い** → SETTINGS で入れる必要がある |
| `recoru_url` | 残骸(**もう誰も読まない**。消すコードは足していない) |
...
| `ヨスガへ共有` | **`過去情報をヨスガへ`** |
| `観測日記を頼む` | **`観察日誌を要求`** |
| `セッション記録を頼む` | **`ヨスガへ総括を要求`** |

**何をするか**ではなく**ヨスガに何が届くか**で呼ぶ形に揃った。
`朝の準備` と `ヨスガへ共有` は役目が紛らわしかった(どちらもヨスガへ渡す)が、
新しい名なら **現況 = 現状報告 / Vault の過去ログ = 過去情報** と名前で分かれる。

追随させた箇所——**放っておくと案内が実在しないボタンを指す**:

- `SettingsScreen` の「ヨスガの会話」欄の説明(夜の2ボタンを名前で指していた)
- `ConsoleScreen` のコメント2箇所(朝が転んだときの逃げ先を `ヨスガへ共有` と書いていた)
- `docs/how-to-use.html`(13箇所)/ `docs/yosuga_prompt.md`(3箇所)/ `CLAUDE.md`

⚠ **WORKLOG の過去の日付節は旧名のまま**にした。当時そう呼んでいたのは事実で、
書き換えると「いつ何と呼んでいたか」が消える。冒頭の再開ポイントにだけ注記を足した。

`ObsidianContextScreen` の画面タイトル2箇所も `過去情報をヨスガへ` へ揃えた
(押したボタンと開いた画面の見出しが食い違わないように)。

### ⚠ 夜の2つを共有インテントへ変えた(同じ日のうちの差し戻し)

**シロさんの指摘**——「日誌やまとめを要求したとき、チャット欄にプロンプトが
貼られてないんじゃないかな。コピーしてるだけで」。**そのとおりだった。**
設計どおりの動作だが、設計のほうが望ましくなかった。

朝と夜で**別の仕組み**を使っていたのが原因:

| | 朝 | 夜(旧) |
|---|---|---|
| 経路 | `ACTION_SEND`(`EXTRA_TEXT`) | `ACTION_VIEW`(URL の `?q=`) |
| 上限 | **約1MB**(Binder) | **2000字** = 日本語 約200字 |
| 行き先 | ChatGPT を名指し。**会話は選べない** | **会話を指定できる** |

朝の現況31KBが欠けずに届いていたのは、**URL を使っていなかったから**。
夜だけが URL 経路で、指示文(640〜1434字)は載らず入力欄は空のままだった。

**この2つは両立しない。** 会話を指定できるのは URL だけで、
長さを運べるのは共有インテントだけ。シロさんの判断で
**「入力欄に必ず入ること」を取り、会話の指定を捨てた**。

→ 夜も `sendToChatGpt` を使う。**毎回まっさらな新しい会話が開く。**

**引き換えに手当てが要った。** 新しい会話には朝の前置きが無いので、
ヨスガは自分が何者かも知らない。共有インテントなら長さの制限が無いので、
`YosugaPrompt.NIGHT_HEADER` を新設して毎回同梱する:

- 役割(「あなたは『ヨスガ』」)
- **材料が無いなら書き始めず、一度だけ聞き返す**
- 覚えている風に埋めない・推測で作らない

⚠ **ここが落ちても動いてしまう。** それらしい嘘の日誌が出て、
承認を挟まず Obsidian に残る。だから `YosugaPromptTest` に2件足して守った
(前置きが付いているか / 材料が無いときの指示が入っているか)。テスト **429 → 431件**。

指示文の材料の記述も直した——「材料は、この会話でのやりとりです」は
新しい会話では嘘になるので、「シロさんが今日について話した内容」へ。

**設定「ヨスガの会話」は退避経路へ降格。** ChatGPT アプリが無い端末でだけ使う。
⚠ 事実上ほぼ使われない欄が残っている。消すかは要判断
(「押せて何も起きない口を作らない」という既存の方針には反している)。

### 実機で使ってみて出た2つの直し

**1. 設定「ヨスガの会話」を撤去した**(risk-018 解消)

夜を共有インテントへ変えた時点で、この URL を読む経路は
「ChatGPT アプリが入っていない端末」だけになっていた。**朝に置いて、
夜に退避経路へ降格し、同じ日のうちに消した欄**。
「押せて何も起きない口を作らない」という既存の方針に従う。

- `UserPreferencesRepository` の Flow / setter / キーを削除
- `SettingsViewModel` の StateFlow / save を削除、`SettingsScreen` の欄ごと削除
- `AssistantUiState.yosugaConversationUrl` と `combine` の1本を削除
- `ConsoleScreen` の退避先は `ChatGptLink.DEFAULT_URL` を直接使う
- ⚠ DataStore に旧キー(`yosuga_conversation_url` / `recoru_url`)は残るが**誰も読まない**

**2. 「今日は何がありましたか」と聞き返すのをやめた**

実機で夜のボタンを押したところ、毎回この確認から始まった。仕様どおりだが、
**シロさんが求めていたのはそこではなかった**——
「今日あったことは、そのセッションが始まる前までのその日の内容をまとめる形で出せばいい」。

材料は**目の前の会話ではなく、その日それまでの会話**。
聞くのではなく ChatGPT の履歴から拾えばいい。`NIGHT_HEADER` を書き換えた:

| 旧 | 新 |
|---|---|
| 材料はこのあと貼るか話す内容 | **材料は「この会話が始まる前までの今日のやりとり」** |
| 足りなければ一度だけ聞く | **確認の質問から始めない** |
| 覚えている風に埋めない | (据え置き)**思い出せない部分は「分かりません」と本文に書く** |

⚠ **捏造の禁止だけは残した。** ChatGPT の履歴参照が効かない設定だと手元が空になり、
それらしい嘘の日誌が出る。しかもセッション記録は**承認を挟まず Obsidian へ入る**ので
そのまま残る。**質問させないことと、嘘を書かせないことは別**なので分けて書いた。
`YosugaPromptTest` は「質問から始めるなと書いてあるか / 推測の禁止が残っているか /
**聞き返しの指示が復活していないか**」を見る。テスト **431 → 432件**。

### ⚠ `measure_prompts.py` が静かに壊れていた

`NIGHT_HEADER` を足した直後、観察日誌の字数が **447 → 187字** と表示された。
減るはずがない(前置きを足したのだから)。

原因は、スクリプトが `"""…"""` を**出てくる順**に
「朝 / 観察日誌 / セッション記録」と決め打ちしていたこと。定数を1つ挟んだだけで
**全部1つずつずれ**、`NIGHT_HEADER` の字数を観察日誌として表示していた。
**エラーは出ない。** 数字がもっともらしいので、疑わなければ通り過ぎる。

→ **名前で取り出す**形に作り直した(`MORNING_HEADER` / `NIGHT_HEADER` /
`fun diary` / `fun session` を目印にする)。取り出せなければ**失敗させる**。
測る観点も「`?q=` に載るか」から「読まれる量として妥当か」へ変えた
——共有インテントに寄せた以上、長さはもう制約ではない。

### 日記の呼び名を「観察日誌」へ統一した

**3通りに割れていた。** `観測日記`(`ContextExporter` / `YosugaPrompt` / 現況JSON)、
`観察日記`(`DiaryEntry` / `DiaryRepository` / `AssistantResponseV2`)、
そして新しいボタン名の `観察日誌`。**71箇所**を `観察日誌` へ寄せた。

⚠ **1つだけ意図的に残した文字列がある。**

```
「観察日記 作成ルール」   ← YosugaPrompt.kt:75, 87 / docs/yosuga_prompt.md / YosugaPromptTest
```

これは **ChatGPT のメモリに保存されている規則の名前そのもの**で、
指示文は「あなたのメモリにある『観察日記 作成ルール』に従ってください」と
**名指しで参照している**。ここを改名すると存在しない名前を指すことになり、
ヨスガは文体規則を見つけられない。**Hub 側の都合で書き換えてよい語ではない。**
メモリ側の名前を変えたときだけ、揃えて変える。

- ⚠ **字数は変わっていない**(`観測日記` / `観察日記` / `観察日誌` はいずれも4字)。
  朝の前置きは **1222字のまま**で、実機で見た `+ PROMPT 1222` と一致する
- **WORKLOG の過去の日付節と `docs/recoru_prompt.md`(廃止済み)は旧語のまま**。
  当時そう書いていた記録なので触らない
- `.yosuga/status.json` / `status.md` は現況なので追随させた(JSON 構文は検証済み)
