...
2. **フォームUIを実機で見る**。`[x]` チェックの当たり判定は
   **2026-07-25 に先回りで 48dp へ広げた**(下記)。見るべきは
   **タスク行で `[x]` とタスク名の間に空く 24dp の隙間が許せるか**。
   広すぎるなら箱だけのときの横幅を戻し、縦だけ 48dp にする。
   チップ(`TerminalChip`)は**まだ約24dp のまま**なので、押しにくければ同じ手当てをする。
3. **レコルの「巡回」と「共有取り込み」を実機で通す**(2026-07-25 追加)。
   先に**カスタムGPT側を設定する**(`docs/recoru_prompt.md` の
   「カスタムGPT側の設定」): Instructions を貼り直す / 会話のきっかけに `巡回` /
   Actions を「常に許可」。そのうえで開いて一言 →
   回答を **共有 → Yosuga Hub** → `[ IMPORT ]`。見るのは:
   - **挨拶だけで巡回が走るか**(合図を打たずに済むか)
   - 途中で聞き返さずに走り切るか
   - 未整理文書が0件の日に、空のJSONを出さずに済むか
   - **ChatGPT の共有シートに Yosuga Hub が出るか**(出なければ経路ごと没)
   - 共有された本文から JSON が抜き出せるか(実行報告が後ろに付いた形)
     ※ 経路自体は 2026-07-25 にエミュレータの共有インテントで確認済み。
       残るのは「ChatGPT の共有シートに出るか」だけ。
4. **実機(スマホ)側**: Obsidian + Remotely Save で Dropbox 同期の構成
...
- ~~**UI の細部**: 戻るアイコン / AlertDialog~~ → **2026-07-24 統一・実機確認済み**。
  ...
  チェックの当たり判定だけ 2026-07-25 に 48dp へ広げた(**これも実機未確認**)。
- **レコルが新しい projects.json を読むところ**が未確認(上記 1.)。
  ロリポップへの push までは Toast で確認済み。
- **共有シート取り込みが実機未確認**(2026-07-25)。
  ChatGPT の共有シートに Yosuga Hub が出るかは実機でしか分からない。
- **レコル自身がサーバーへ保存する経路は未実装**(2026-07-25)。
  共有が通れば不要かもしれない(手数がほぼ同じでサーバー変更が要らないため)。
  作るなら `inbox.php`(⚠ `upload.php` の流用は禁止・`projects.json` を潰す)+
  `openapi.yaml` に POST + アプリの `PULL INBOX`。詳細は 2026-07-25 の項。
- **クリップボード自動取り込みは採らなかった**(Android 12+ で毎回トーストが出るため)。
  やるならボタン式(`PASTE CLIP`)。詳細は 2026-07-25 の項。
...
### 2026-07-24〜25 にやったこと(新しい順)

| コミット | 内容 |
|---|---|
| `84b2469` | `[x]` チェックの当たり判定を 48dp へ |
