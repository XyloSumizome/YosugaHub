...
- 単体テスト **410件** 通過 / マイグレーションテスト **5件** 通過(実機)。Room は **v8**。新規ライブラリなし。
- ビルドは Windows 側 JBR
...
- **ヨスガ**(通常ChatGPT・**カスタムGPTではない**) = 会話の相棒。
  **観測日記を書く**。**タスク・知識アイテムの提案JSONも直接出す**(2026-07-26〜)。
  Instructions 欄が無いので**メモリに覚えさせる**(現在 4128字)。
  ⚠ 観察日記の文体規則は**ヨスガのメモリ側が正**。`docs/yosuga_prompt.md` には
  **Hub へ取り込む形式だけ**を書く(二重に書かない)。
- **レコル**(カスタムGPT+Actions) = **記録タブに手打ちした未整理文書の仕分けだけ**。
  朝の近況からは外れた。`documents` が0件ならコンソールに `レコルを開く` も出ない。
  ⚠ Instructions は **8000字上限**(現在 6339字)。編集したら
  `python3 scripts/measure_prompts.py` で測る。
  ⚠ **ウェブ検索機能は OFF のまま**にする(ONだと「巡回」を外部サイト巡回と解釈する)。
- 各ゲームの **Claude Code** = `.yosuga/notes/` と `.yosuga/status.json` を書く。
  **Frontmatter で分類まで済ませる**(Hub は振り分けるだけ / レコルは触らない)。
...
1. **AI 側の指示を貼り直す**(⚠ **朝の経路を変えたので、やらないと明日ズレる**)。
   - **ヨスガ**: `docs/yosuga_prompt.md` の **70行目以降のコードブロック**をコピーし、
     会話に貼って「これを覚えて」。`# 朝の近況(Hubから直接届く)` が新しい節。
   - **レコル**: `docs/recoru_prompt.md` の **41行目以降のコードブロック**で
     Instructions を**丸ごと差し替える**(継ぎ足すと8000字で切れる)。
   - **レコルの「会話のきっかけ」から `Morning Brief` を削除する。**
2. **実運用の夜の側を通す**。制作 → 観測日記 → `> 回答JSON → Hub` → 承認。
   朝の側は 2026-07-26 に通った。
3. アイコンが実機で更新されたか確認(⚠ ランチャーはアイコンをキャッシュする)。
