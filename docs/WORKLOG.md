...
- **表示名が英字になっている**。`adb input text` は日本語を打てないため
  `Kamieru` / `GengeKyou` で作った。**表示名は Vault のフォルダ振り分けに使われる**ので、
  アプリの「編集」で `紙装甲主人公と不死身のカエル` / `げんげきょう` に直すこと。
- **gengenkyo のリポジトリ設定が空**。正しいリポジトリ名とブランチが不明なため入れていない。
