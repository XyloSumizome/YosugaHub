...
⚠ この表示名は 2026-07-27 に **`> ヨスガへ現状報告`** へ改めた(動きは同じ)。
...
1. **実機で新しい経路を通す**(2026-07-27 の変更は**一部だけ確認済み**)。
   - ✅ `> ヨスガへ現状報告` を実機で押し、端末ログに
     `CONTEXT … (+ PROMPT 1222)` が出た。**指示文の同梱は通った**
   1. SETTINGS →「ヨスガの会話」に、その日の会話URL(`https://chatgpt.com/c/…`)を入れる
      ——⚠ **ここが最大の未知**。ChatGPT の Android アプリは共有時に
      `/share/…` のスナップショットURLを作る可能性があり、`/c/…` が取れないと
      夜の2ボタンの前提が崩れる。ブラウザ版のアドレス欄からなら確実
   2. 送信して、**メモリを消しても同じように始まるか**(指示文だけで足りるか)
   3. 夜に `> 観察日誌を要求` / `> ヨスガへ総括を要求` → 貼って送信
