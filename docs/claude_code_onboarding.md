...
**v5(2026-07-24)で役割が1つ増えた。**
`.yosuga/status.json`(= 今の状態のスナップショット)に加えて、
**知識として残す Markdown ノート**を出力する。後半の「## v5: 知識ノートの出力」を参照。
...
- **`recentChanges` の各件に `date`("yyyy-MM-dd")を必ず書く**(2026-07-25〜)。
  ...
  古い変更を消すのは Hub 側の仕事なので、**あなたは消さずに積んでよい**。
...
## v5: 知識ノートの出力(2026-07-24 追加)

### なぜ必要か

Yosuga Hub は v5 で **「情報を確実に運ぶハブ」** になった。Hub は判断も要約もしない。
**意味付け(何の情報で、どのゲームの、どの分類か)は、情報を作る側＝各ゲームの Claude Code が行う。**
Hub 側が中身を読んで分類し直さなくてよい形で出力すること。

`status.json` との違い:

| | status.json | 知識ノート(Markdown) |
|---|---|---|
| 中身 | **今の状態**のスナップショット。上書きされる | **その時点の記録**。積み上がる |
| 読む人 | Yosuga Hub(機械) | 人間 + AI |
| 保存先 | リポジトリ内 | Obsidian Vault |
| 目的 | 進捗表示 | 後から検索・参照できる知識 |

**status.json は引き続き維持すること。**置き換えではなく追加。

### いつ書くか

意味のある作業単位が終わったとき。目安:

- 設計判断をしたとき(なぜそうしたかを残す) → `type: decision`
- まとまった実装が終わったとき → `type: development-log`
- 仕様・設計をまとめたとき → `type: design`
- 調べ物の結果を残したいとき → `type: reference`

**細かすぎるコミット単位では書かない。**1日1〜3件が目安。

### 形式(必須)

先頭に YAML Frontmatter、その下に本文。
...
なぜ変更したか。**ここが一番重要。**後から読むのは「なぜ」であって「何を」ではない。
...
| `tags` | ✅ | 1つ目は必ずゲーム名。以降に type と category を含める |
| `related_files` | 任意 | リポジトリルートからの相対パス。**絶対パスは書かない** |
| `commit` | 任意 | `hash` と `branch` |

### ファイル名

```
YYYY-MM-DD-<短い英語スラッグ>.md
例: 2026-07-24-lighting-falloff.md
```

同じ日に複数書くときは連番ではなくスラッグで区別する。**既存ファイルを上書きしない。**

### 守ること

- **推測を書かない。**不明なことは「未解決事項」に書く
- **秘密情報・APIキー・個人情報・ローカル絶対パスを書かない**
- 本文は原文のまま Hub からヨスガへ渡される。**要約は誰もしない**前提で、読んで意味が通る文章にする
- `## 未解決事項` を省略しない(無ければ「なし」)
...
- 新しい記録は新しいファイルにする。**既存ノートの訂正・追記は書き換えてよい**
  (2026-07-25〜: Hub は更新されたノートを検出し、Vault 側の同じファイルを
  新しい版で上書きする。古い版は残らない)
- ノートを消すと、Hub は「元が消えた」と**報告だけ**する(Vault 側は消さない。
  消すかどうかはシロさんが Obsidian で決める)
