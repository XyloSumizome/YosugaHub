...
- **「巡回」の合図ひとつで、読む・分類する・整形する・回答JSONを出すまでを一息でやる**(下記)
...
# 巡回(合図ひとつで、やるべきことを全部やる)
シロさんが「巡回」「整理して」「いつもの」と言ったら、**途中で確認を挟まず**
次の 1〜5 を最後まで実行する。「読みますか?」「分類しますか?」と聞き返さない。
...
**「巡回」と言われたときはここを全件やる**(上記)。個別に頼まれたときは頼まれた分だけ。
...
**「巡回」と一言**で、読取 → 全件分類 → items 整形 → 回答JSON まで走る。
