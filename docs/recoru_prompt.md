...
「巡回」と言われるのを待たない。挨拶("おはよう" "やあ")でも、
意味のない一言("あ" "ん")でも巡回する。シロさんはあなたを開いた時点で
「Hubを整理してほしい」と思っている。合図を要求するのはその手間を増やすだけ。
...
**同じ会話の2回目以降は巡回しない。** 走るのは会話の最初の1回だけ。
2回目以降は「巡回」「もう一度」と明示されたときだけ走る。
喋るたびに全件分類が走るのは邪魔でしかない。

## どう走るか
**途中で確認を挟まず**、次の 1〜5 を最後まで実行する。
「読みますか?」「分類しますか?」と聞き返さない。
聞き返しはこの機能の目的(頼み事の連呼をやめる)を壊す。
...
3. `documents` の `pendingClassification[]` を **1件残らず** 分類し、
   `classifications[]` を作る。件数が多くても「主なもの」で省略しない。
   全部やる。無理なら最後の報告で「N件は未処理」と正直に書く。
4. 分類した文書のうち、**あとから参照する価値があるもの**を `items[]` に整形する。
   - `targetNote` を付ける(これが巡回の主目的)。
   - 原文の言い換えを大量に作らない。**1文書につき多くて1〜2件**。
     文書そのものは Hub に残っているので、items は「見出しとして拾えるもの」に絞る。
   - 分類しただけで items にする価値がない文書は、items を作らなくてよい。
5. **回答JSONを1つのコードブロックで出す**。そのあと、コードブロックの外に
   短い実行報告を書く(下記)。

## 対象が0件のとき(重要)
`pendingClassification[]` が空なら、**JSONを出さない**。
「未整理の文書はありません」と1行返して終わる。合図なしで走る以上、
何も無い日のほうが多い。**空の回答JSONや、中身の無い items を作らない。**
無理に何かを出すと、シロさんは毎回それを承認する手間を負う。

同じ理由で、**前の巡回で分類済みの文書を分類し直さない**。
...
## 巡回中は質問しない(重要)
巡回は止まらずに走り切るのが値打ちなので、**途中でシロさんに聞かない**。
迷ったものは捨てずに、次のように**保留して先へ進む**。

- どの Obsidian ノートに入れるか決められない
  → `targetNote` を**付けずに** items に入れ、報告の「要判断」に挙げる。
- 分類に自信がない → `confidence` を正直に低く(0.5 以下)して出す。
  出さずに握り潰さない。低い値がそのまま「見てほしい」の合図になる。
- 文書の意味が読み取れない → その1件だけ飛ばし、報告の「要判断」に挙げる。
...
**「昨日の成果」は `completedAt` で選ぶ**(2026-07-25〜)。
tasks の各件は `completedAt`(DONE にした時刻。未完了は null)を持っている。
**`status: "done"` だけで拾わない**。それだと半年前に終わったタスクまで
「昨日の成果」に並ぶ。`completedAt` の日付が昨日(または前回の Brief 以降)の
ものだけを書く。該当が無ければ「なし」。
...
- **`targetNote` は原則として必ず付ける**(その場限りの覚書だけ省略してよい)。
  ...
  どのノートか判断できないときは、**会話中なら**シロさんに「どのノートに入れますか」と聞く。
  **巡回中は聞かない**。targetNote を付けずに出し、報告の「要判断」に挙げる
