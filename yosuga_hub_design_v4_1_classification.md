...
> **状態: 設計指示を受領・未実装(2026-07-23)**。
> 本書は v4(`yosuga_hub_android_design_v4.md`)への追加仕様。
> 実装着手前に、末尾「未確定の論点」をユーザーと合意すること。
...
1. **今回の実装範囲**: フル実装(データ層+同期+取込+レビューUI)/ データ層まで / 設計記録のみ。
2. **文書の作成入口**: 記録タブに「文書」セクションを新設(推奨)/ 他アプリからの共有インテントも対応するか。
3. **分類結果の受け取り方**: 回答JSON v2 の `proposals.classifications[]` に追加(推奨。既存の
   取込・承認機構をそのまま再利用できる)/ サーバーに専用受口PHPを追加してポーリング。
4. 承認確定した文書を `knowledge_items` へ昇格させるか、documents のままにするか。
5. `related_entities` の `type: "feature"` は既存 EntityType(project/person/tech/gear/event/other)
   に無い。EntityType を拡張するか、文書側だけ自由文字列にするか。
